...
| `DOCUSIGN_QUEUE_ENABLED` (tuỳ chọn) | Không phải thông tin từ DocuSign, là flag nội bộ | Đặt `true` nếu bạn muốn dùng BullMQ để xử lý DocuSign ở background; để trống hoặc `false` thì backend sẽ gọi DocuSign ngay trong request (không cần worker). |
...
5. **Gửi DocuSign:** Endpoint thứ hai dựng tài liệu từ snapshot vừa “khóa”, thêm danh sách signer (freelancer → client → platform) rồi gửi qua DocuSign. Theo mặc định, backend xử lý trực tiếp ngay trong request nên email sẽ đi ngay cả khi bạn chưa bật worker. Nếu muốn tách hẳn sang background job, đặt `DOCUSIGN_QUEUE_ENABLED=true`, khởi chạy Redis và chạy `npm run worker:docusign`; khi đó endpoint chỉ enqueue và worker sẽ gọi DocuSign.
...
3. **Cập nhật DB & log sự kiện**:
   ...
   * Mọi webhook đều được ghi vào bảng `webhook_event_log` (type `docusign.connect`) trước khi xử lý; trường `processed` giúp bạn biết callback nào đã áp dụng thành công vào DB. Có thể query bảng này hoặc tail log console để debug khi DocuSign báo gửi thành công nhưng trạng thái chưa đổi.
