...
| Danh sách trọng tài | `GET /admin/disputes/arbitrators`                                                        | Admin                           | Trả về `{ data: ArbitratorSummary[] }` với từng trọng tài gồm `id`, `name`, `email`. Chỉ liệt kê user đang `isActive` và có `role = ARBITRATOR`. |
| Chỉ định trọng tài  | `POST /admin/disputes/:disputeId/arbitrator`                                             | Admin                           | Body `{ arbitratorId: string }`. Yêu cầu dispute đã được khóa (`lockedAt` khác null). Cập nhật `arbitratorId`, `arbitratorAssignedAt` và trả về dispute detail mới nhất. |
...
| Lấy danh sách dossier | `GET /admin/disputes/:disputeId/dossiers?page=1&limit=20`                              | Admin
                           | Trả về `{ disputeId, page, limit, total, hasMore, data }` với `data` sắp xếp giảm dần theo `version`, bao gồm metadata và `payload` snapshot để admin/trọng tài xem chi tiết hoặc render PDF. `limit` tối đa 50. |
