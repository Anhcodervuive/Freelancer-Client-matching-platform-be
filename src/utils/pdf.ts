...
class TrueTypeFont {
    ...
    private getGlyphIdFromCmap(codePoint: number): number | null {
        ...
        for (let index = 0; index < segments.length; index += 1) {
            ...
            const offset =
                this.cmap.idRangeOffsetOffset +
                index * 2 +
                segment.rangeOffset +
                (codePoint - segment.start) * 2
            if (offset < this.cmap.glyphIndexArrayOffset || offset >= this.cmap.glyphIndexArrayOffset + this.cmap.glyphIndexArrayLength) {
                return null
            }
...
class EmbeddedFont {
    ...
    constructor(
        ...
        font: TrueTypeFont,
        private readonly isItalicStyle: boolean
    ) {
        this.font = font
    }
    ...
    private buildFontDescriptor(descriptorId: number, fontFileId: number, baseName: string) {
        ...
        const descriptor = `<< /Type /FontDescriptor /FontName /${baseName} /Flags ${flags} /FontBBox [${bbox.join(' ')}] /ItalicAngle ${this.font.italicAngle.toFixed(2)} /Ascent ${ascent} /Descent ${descent} /CapHeight ${capHeight} /XHeight ${xHeight} /StemV ${stemV} /AvgWidth ${avgWidth} /MaxWidth ${maxWidth} /FontFile2 ${fontFileId} 0 R >>`
    ...
    private drawTable(table: PdfTableContent) {
        ...
        rows.forEach((row, rowIndex) => {
            ...
            const lineSets = row.cells.map((cell, columnIndex) => {
                const availableWidth = Math.max((columnWidths[columnIndex] ?? CONTENT_WIDTH / table.columns.length) - paddingX * 2, row.fontSize)
