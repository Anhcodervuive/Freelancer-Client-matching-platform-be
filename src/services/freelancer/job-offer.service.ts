...
const listJobOffers = async (freelancerUserId: string, filters: FreelancerJobOfferFilterInput) => {
	...
	const andConditions: Prisma.JobOfferWhereInput[] = [{ status: { not: JobOfferStatus.DRAFT } }]
	...
	if (payload.action === 'ACCEPT') {
		...
		const result = await prismaClient.$transaction(async tx => {
			if (offer.jobId) {
				...
				if (hiredOffer) {
					throw new BadRequestException('Công việc đã có freelancer được thuê trước đó', ErrorCode.PARAM_QUERY_ERROR)
				}
			}
			...
			const contract = await tx.contract.create({
				data: {
					...
					platformTermsSnapshot: activeTerms.body === null ? Prisma.JsonNull : activeTerms.body
				}
			})
			...
			await Promise.all(
				result.withdrawnOffers.map(withdrawnOffer =>
					notifyFreelancerOfferWithdrawn(result.updatedOffer.clientId, withdrawnOffer, {
						action: 'withdrawn',
						withdrawReason: withdrawnOffer.withdrawReason ?? AUTO_WITHDRAW_REASON_HIRED,
						autoWithdraw: true
					})
				)
			)
