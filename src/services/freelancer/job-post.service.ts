...
const sanitizeClientSummary = (
        client: PublicJobSummaryPayload['client'] | PublicJobDetailPayload['client']
) => {
...
const buildPublicJobWhere = (filters: FreelancerJobPostFilterInput): Prisma.JobPostWhereInput => {
        ...
        if (filters.paymentModes && filters.paymentModes.length > 0) {
                        where.paymentMode =
                                filters.paymentModes.length === 1 ? filters.paymentModes[0]! : { in: filters.paymentModes }
        }

        if (filters.formVersions && filters.formVersions.length > 0) {
                        where.formVersion =
                                filters.formVersions.length === 1 ? filters.formVersions[0]! : { in: filters.formVersions }
        }
        ...
        if (filters.experienceLevels && filters.experienceLevels.length > 0) {
                        where.experienceLevel =
                                filters.experienceLevels.length === 1
                                        ...
                                        : { in: filters.experienceLevels }
        }

        if (filters.locationTypes && filters.locationTypes.length > 0) {
                        where.locationType =
                                filters.locationTypes.length === 1
                                        ? filters.locationTypes[0]!
