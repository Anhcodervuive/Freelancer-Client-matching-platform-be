...
const syncJobAttachments = async (
	...
	const removedAssetIds = existingLinks.map(link => link.assetId).filter(assetId => !uniqueAssetIds.includes(assetId))
...
const buildJobPostWhere = (params: {
	...
	if (params.experienceLevels && params.experienceLevels.length > 0) {
		where.experienceLevel =
			params.experienceLevels.length === 1 ? params.experienceLevels[0]! : { in: params.experienceLevels }
	}

	if (params.locationTypes && params.locationTypes.length > 0) {
		where.locationType = params.locationTypes.length === 1 ? params.locationTypes[0]! : { in: params.locationTypes }
	}
...
const updateJobPost = async (
	...
	let shouldQueueModeration =
		(publishRequested || contentChanged) && (publishRequested || AUTO_REVIEW_STATUSES.has(job.status))
	...
	if (input.preferredLocations !== undefined) {
		data.preferredLocations =
			input.preferredLocations === null ? Prisma.DbNull : (input.preferredLocations as Prisma.InputJsonValue)
	}

	if (input.customTerms !== undefined) {
		data.customTerms = input.customTerms === null ? Prisma.DbNull : (input.customTerms as Prisma.InputJsonValue)
	}
	...
	try {
		await prismaClient.$transaction(async tx => {
			...
			if (attachmentsToPersist !== undefined) {
				await syncJobAttachments(tx, jobId, attachmentsToPersist, clientUserId, assetsToCleanup)
			}
		})
	} catch (error) {
		await cleanupUploadedAttachments(uploadedAttachments)
	...
	if (filters.mine) {
		if (!viewerId) {
			throw new UnauthorizedException(
				'Bạn cần đăng nhập để xem danh sách job post của mình',
