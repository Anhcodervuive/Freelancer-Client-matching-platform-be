...
const deleteJobInvitation = async (invitationId: string, clientUserId: string) => {
	...
	if (invitation.status !== JobInvitationStatus.SENT && invitation.status !== JobInvitationStatus.EXPIRED) {
		throw new BadRequestException('Chỉ có thể xóa lời mời khi freelancer chưa phản hồi', ErrorCode.PARAM_QUERY_ERROR)
	}
