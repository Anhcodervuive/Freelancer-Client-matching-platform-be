import type { ChatMessageType, Role } from '@prisma/client'
...
const RATE_LIMIT_KEY = (userId: string, windowSeconds: number) => `chat:rate:${userId}:${Math.floor(Date.now() / (windowSeconds * 1000))}`
