...
export async function seedMatchInteractions() {
        return runStep('Match interactions', async () => {
                ...
                for (const seed of MATCH_INTERACTIONS) {
                        ...
                        const freelancerId = seed.freelancerEmail
                                ? userByEmail.get(seed.freelancerEmail)?.id
                                : undefined
                        const clientId = seed.clientEmail ? userByEmail.get(seed.clientEmail)?.id : undefined
                        const jobClientId = seed.jobClientEmail ? userByEmail.get(seed.jobClientEmail)?.id : undefined
